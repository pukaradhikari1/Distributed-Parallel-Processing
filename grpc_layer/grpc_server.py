...
def find_orchestrator():
    ...
    BROADCAST_TIMEOUT = 15  # wait 15 seconds for broadcast before trying fallback
    ...
    while True:
        try:
            ...
            if msg.startswith("ORCHESTRATOR:"):
                ip = msg.split(":")[1]
                print(f"\nFound Orchestrator at {ip}")
                client.close()
                return ip
        except socket.timeout:
            # If WSL and broadcast timed out — try getting gateway IP directly
            if is_wsl and (time.time() - start_time) > BROADCAST_TIMEOUT:
                print(f"\nWSL broadcast timeout — trying direct gateway discovery...")
                client.close()
                return get_orchestrator_via_gateway()
            continue
